...
template klogf(string fmt, string file = __FILE__, int line = __LINE__) {
    void klogf(Args...)(Args args) {
        klogImpl!(fmt, file, line, true)(LogLevel.Info, args);
    }
}
 ...
 */
template
kpanic(string fmt, string file = __FILE__, int line = __LINE__) {
...
void
kassert(bool cond, const(char)* msg, string file = __FILE__, int line = __LINE__) {
    if (!cond) {
        LineBuf b;
