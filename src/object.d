...
const KERNEL_VMA       = 0xffffffffff000000;
const KERNEL_PMA       = 0x0020_0000;

const TIMER_FREQ       = 100;
const USECS_PER_TICK   = 1_000_000 / TIMER_FREQ;
const MSECS_PER_TICK   = 1_000 / TIMER_FREQ;
...
private bool
areClassInfosEqual( scope const ClassInfo a, scope const ClassInfo b ) pure nothrow @safe @nogc
{
    if( a is b )
        return true;

    if( (a.m_flags & TypeInfo_Class.ClassFlags.hasNameSig) && (b.m_flags & TypeInfo_Class.ClassFlags.hasNameSig) )
...
private bool
classinfo_isbaseof( scope ClassInfo actual, scope const ClassInfo target, scope ref size_t offset ) @trusted
{
    if( !actual || !target )
        return false;

    if( areClassInfosEqual( actual, target ) )
        return true;

    do {
        if( actual.base && areClassInfosEqual( actual.base, target ) )
            return true;

        // Search interface trees depth-first to accumulate the correct
        // pointer delta for class<->interface and interface<->interface casts.
        foreach( iface; actual.interfaces ) {
            if( areClassInfosEqual( iface.classinfo, target ) || classinfo_isbaseof( iface.classinfo, target, offset ) ) {
                offset += iface.offset;
                return true;
            }
        }
...
private void*
class_cast_impl( const return scope Object o, const ClassInfo targetClass ) @trusted
{
    if( !o || !targetClass )
        return null;

    auto actual = typeid(o);
    int delta = cast(int)actual.depth;

    if( delta && targetClass.depth ) {
        delta -= targetClass.depth;
        if( delta < 0 )
            return null;
        ...
        return areClassInfosEqual( actual, targetClass ) ? cast(void*)o : null;
    }

    do {
        if( areClassInfosEqual( actual, targetClass ) )
            return cast(void*)o;
        actual = actual.base;
    } while( actual );
...
private void*
dynamic_cast_impl( const return scope Object o, const ClassInfo targetClass ) @trusted
{
    if( !o || !targetClass )
        return null;

    size_t offset = 0;
    return classinfo_isbaseof( typeid(o), targetClass, offset ) ? cast(void*)o + offset : null;
}

private void*
interface_cast_impl( void* p, const ClassInfo targetClass ) @trusted
{
    if( !p || !targetClass )
        return null;
    ...
    return dynamic_cast_impl( o, targetClass );
}

extern(C) Object _d_dynamic_cast(Object o, TypeInfo_Class targetClass)
{
    return cast(Object)dynamic_cast_impl( o, targetClass );
}
...
{
    return interface_cast_impl( p, targetClass );
}
...
{
    static if( is(Tsrc == Tret) ) {
        return *cast(Tret*)&o;
    } else static if( is(Tsrc == class) && is(Tret == interface) ) {
        return cast(Tret)dynamic_cast_impl( cast(Object)o, typeid(Tret).info );
    } else static if( is(Tsrc == class) && is(Tret == class) ) {
        static if( is(Tret : Tsrc) ) {
            return cast(Tret)class_cast_impl( cast(Object)o, typeid(Tret).info );
        } else {
            return null;
        }
    } else static if( is(Tsrc == interface) ) {
        return cast(Tret)interface_cast_impl( cast(void*)o, typeid(Tret).info );
    } else {
        return null;
    }
}
...
void full_fence()
{
    ...
    {
        // Lower directly to LLVM's sequentially-consistent fence when druntime
