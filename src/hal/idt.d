...
struct Context {
    ...
    /* CPU interrupt frame for same-CPL handlers: RIP, CS, RFLAGS.
     * RSP/SS are only present when the CPU changes privilege level. */
    u64 rip;
    u64 cs;
    u64 rflags;
    u64 rsp;
...
idt_init() {
    ...
    wrmsr( 0xC0000081, (cast(u64)USER_CS << 48) | (cast(u64)KERNEL_CS << 32) );
    wrmsr( 0xC0000082, cast(u64)&syscall_entry );
    wrmsr( 0xC0000084, 0 ); // mask flags if needed
}
...
isr_common() {
    asm {
        naked;
        /* For ring-0 interrupts the CPU frame is RIP, CS, RFLAGS.
         * RSP/SS are only added on privilege transitions or IST switches. */
        push RAX;
        push RCX;
        push RDX;
        push RBX;
...
isr_handler( Context* ctx ) {
    ...
    if( vec < 32 ) {
        ...
        kprintf( "  vector       =   {016i} | error_code     =   {016i}\n", vec, ctx.error_code );
        kprintf( "  RIP          = 0x{016x} | EFLAGS         = 0x{016x}\n", ctx.rip, ctx.rflags );
        kprintf( "  CR2          = 0x{16x}\n", cr2 );
